...
// Biztonságos localStorage helper funkciók
...
export function FigmaIntegrationPanel({ onComplete }: FigmaIntegrationPanelProps) {
  ...
  // useEffect hook a localStorage értékek betöltéséhez
  useEffect(() => {
    const token = getFromLocalStorage(FIGMA_CONFIG.STORAGE_KEY)
    const demoMode = getFromLocalStorage("figma-demo-mode") === "true"
  ...
  const handleProcessFile = async () => {
    ...
    if (!isDemoMode) {
      ...
      const token = accessToken || storedToken || FIGMA_CONFIG.TOKEN
      if (!token) {
    ...
    try {
      ...
      if (accessToken) {
        setToLocalStorage(FIGMA_CONFIG.STORAGE_KEY, accessToken)
...
.figma-component p {
  ...
  const toggleDemoMode = () => {
    ...
    if (newDemoMode) {
      setToLocalStorage("figma-demo-mode", "true")
      setToLocalStorage("figma-connection-status", "true")
    } else {
      removeFromLocalStorage("figma-demo-mode")
      setToLocalStorage("figma-connection-status", "false")
    }
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="access-token">
                  Figma Access Token
                  <Badge
                    variant={accessToken || storedToken ? "default" : "destructive"}
                    className="ml-2"
                  >
                    {accessToken || storedToken ? "Configured" : "Required"}
                  </Badge>
                </Label>
                <Input
                  id="access-token"
